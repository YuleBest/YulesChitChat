---
title: "CSS 伪元素介绍 | ::before 和 ::after 如何提升你的网页设计"
...
在前端开发的世界里，有一个原则至关重要：结构与样式分离。我们希望 HTML 代码保持干净、语义化，而不被大量的装饰性 `<div>` 或 `<span>` 标签所污染。

这时候，CSS 伪元素 _(Pseudo-elements)_ 就闪亮登场了。它们就像是隐藏在 CSS 中的「幽灵元素」，不仅能帮你保持 HTML 的整洁，还能创造出令人惊叹的视觉效果。
...
使用这两个伪元素时，有一个绝对不能忘记的规则：`::before` 和 `::after` 必须配合 `content` 属性使用（哪怕是空字符串 `content: "";`）。如果省略 `content`，伪元素将不会显示。
...
这是 `::after` 最经典的用法之一。虽然现代布局多用 Flexbox 和 Grid，但在维护旧代码时，这个技巧依然是标配。
...
想做像报纸一样的“首字下沉”效果？不需要把第一个字包在 `<span>` 里。
...
### 可访问性 (Accessibility) 警示

虽然伪元素很强大，但在屏幕阅读器（Screen Readers）的解析上存在差异。

- 不要将重要内容放在伪元素中：如果是用户必须知道的信息（如价格、状态），请直接写在 HTML 里。
- 伪元素最适合用于装饰性内容（如图标、分割线、引号）。
